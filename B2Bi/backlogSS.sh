...
dofind $D/*.arc -mtime -7 -cmin +"$time" 2>/dev/null |wc -l`
 BL=`
 if [[ $BL != '0' ]];
 then
   echo `basename $D`  $BL
...
echo "(/app/sword/shared/mounts/nfsserver/ComServer1/JMS/node/b2biprdcom1_shield/queuename)"
for i in `ls $JMS`;
...
echo "(/app/sword/shared/mounts/nfsserver/ComServer1/JMS/node/b2biprdcom1_1/queuename)"
for i in `ls $JMS`;
do echo -e "\033[41;37m$i\033[0m";
for queue_name in `ls -d $JMS/$i/b2biprdcom1_1/Scintilla* 2>/dev/null`;do
